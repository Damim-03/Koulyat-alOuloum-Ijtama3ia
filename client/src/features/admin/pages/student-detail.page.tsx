...
const STATUS: Record<string, { label: string; cls: string }> = {
  ...
  accepted: { label: "مقبول", cls: "bg-emerald-100 text-emerald-600" },
...
export function AdminStudentDetailPage() {
  ...
  if (!student) {
    return (
      <div className="font-body py-24 text-center">
        <p className="text-sm text-clay">لم يتم العثور على الطالب</p>
        <button
          onClick={backToList}
        ...
        >
          <ArrowRight size={16} /> العودة إلى الطلبة
        </button>
      </div>
    );
  }
  ...
  const info: {
    ...
    { icon: Layers, label: "التخصّص", value: spec?.name ?? "\u2014" },
    { icon: Network, label: "الشعبة", value: filiere?.name ?? "\u2014" },
    { icon: Building2, label: "القسم", value: dept?.name ?? "\u2014" },
    { icon: GraduationCap, label: "الكلّية", value: faculty?.name ?? "\u2014" },
    {
      icon: CalendarDays,
      label: "السنة الجامعية",
      value: student.academicYear?.title ?? "\u2014",
    },
    { icon: Clock, label: "آخر دخول", value: fmtDate(u.lastLoginAt) },
    {
      icon: CalendarDays,
  ...
  return (
    <div className="font-body mx-auto max-w-5xl">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-forest/10 bg-cream-card shadow-[0_10px_40px_rgba(38,66,61,0.10)]">
        ...
        <div className="px-6 pb-6 sm:px-8">
          ...
          <div className="-mt-16 flex">
            <div className="relative">
              ...
              <span
                ...
                title={u.status === "active" ? "نشط" : "موقوف"}
              />
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <MiniStat
              icon={BadgeCheck}
              value={u.isVerified ? "موثّق" : "غير موثّق"}
              label="حالة الحساب"
      ...
      <Section
        icon={FileText}
        title="طلبات المواضيع الفردية"
        count={applications.length}
      >
        {applications.length === 0 ? (
          <Empty icon={FileText} text="لا توجد طلبات فردية." />
        ) : (
            ...
            {applications.map((a: any) => (
              <LinkRow
                ...
                meta={[
                  ...
                  a.priority != null && `الأولوية ${a.priority}`,
                ]}
                metaIcons={[User, Hash]}
          ...
          <ul className="space-y-2">
            {ledRequests.map((r: any) => (
              <LinkRow
                ...
                status={r.status}
                meta={[
                  r.members && `${r.members.length} عضو`,
              ...
              <LinkRow
                ...
                meta={[
                  m.request?.leader?.user &&
                    `القائد: ${personName(m.request.leader.user)}`,
                ]}
                metaIcons={[Crown]}
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-clay">
                        ...
                        {g.defense ? (
                          <span className="inline-flex items-center gap-1 text-gold">
                            <MessagesSquare size={12} /> مناقشة:{" "}
                            {fmtDate(g.defense.date)}
                            {g.defense.room ? ` · ${g.defense.room}` : ""}
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1">
                            <MessagesSquare size={12} /> لا مناقشة بعد
                          </span>
                        )}
                      </div>
                    </div>
      ...
      {!hasActivity && (
        <div className="mt-6 grid place-items-center gap-2 rounded-3xl border border-forest/10 bg-cream-card py-14 text-center shadow-[0_4px_20px_rgba(38,66,61,0.05)]">
          ...
          <p className="text-sm font-medium text-forest">لا نشاط بعد</p>
          <p className="text-xs text-clay">
            لم يقدّم هذا الطالب أي طلبات ولا ينتمي لأي مشروع حتى الآن.
          </p>
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        tone="danger"
        title="حذف الطالب"
        message={`هل أنت متأكّد من حذف «${name}»؟ سيُحذَف حسابه وبياناته المرتبطة. لا يمكن التراجع.`}
