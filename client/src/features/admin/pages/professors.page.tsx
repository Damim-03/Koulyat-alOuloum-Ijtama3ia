...
function fullName(p: Professor) {
  const u = p.user;
  return [u?.firstName, u?.lastName].filter(Boolean).join(" ") || p.universityEmail || "\u2014";
}
...
export function AdminProfessorsPage() {
  ...
  const { data, isLoading } = useProfessors({ page, limit: PAGE_SIZE, ...applied });
  ...
  const topicsOnPage = professors.reduce((acc, p) => acc + (p._count?.topics ?? 0), 0);
  ...
  return (
    <div className="font-body">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-serif text-2xl font-bold text-forest">{t("admin.professorsTitle")}</h1>
          <p className="mt-1 text-sm text-clay">{t("admin.professorsSubtitle")}</p>
        </div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatTile icon={UserCog} value={total} label={t("admin.totalProfessors")} tint="bg-soft-sage/30 text-forest" />
        <StatTile icon={FileText} value={topicsOnPage} label={t("admin.supervisedTopics")} tint="bg-emerald-100 text-emerald-600" />
        <StatTile icon={FolderKanban} value="\u2014" label={t("admin.currentProjects")} tint="bg-gold/15 text-gold" />
        <StatTile icon={BookOpen} value="\u2014" label={t("admin.assignedCourses")} tint="bg-clay/15 text-clay" />
      </div>
          ...
          <table className="w-full text-right">
            <thead>
              <tr className="bg-forest text-cream">
                ...
                <th className="px-5 py-3 text-xs font-medium">{t("admin.universityEmail")}</th>
                <th className="px-5 py-3 text-xs font-medium">{t("admin.department")}</th>
                <th className="px-5 py-3 text-xs font-medium">{t("admin.topicsCount")}</th>
            ...
            <tbody className="divide-y divide-forest/10">
              ...
              {!isLoading && professors.length === 0 && (
                <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-clay">{t("admin.noProfessors")}</td></tr>
              )}

              {professors.map((p) => (
                <tr key={p.id} className="transition-colors hover:bg-forest/[0.03]">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-forest">{fullName(p)}</p>
                        <p className="text-[11px] text-clay">{p.employeeNumber}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5 text-sm text-clay" dir="ltr">{p.universityEmail}</td>
                  <td className="px-5 py-3.5 text-sm text-clay">{p.department?.name ?? "\u2014"}</td>
                  <td className="px-5 py-3.5">
