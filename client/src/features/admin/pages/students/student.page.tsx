...
export function AdminStudentsPage() {
  ...
  const params = useMemo(
    () => ({
      page,
      limit: PAGE_SIZE,
      // `search` is the person's name; the registration number is its own field.
  ...
  return (
    <div className="font-body">
      ...
      <div className="mb-4 overflow-hidden rounded-2xl border border-forest/10 bg-cream-card shadow-[0_4px_20px_rgba(38,66,61,0.05)]">
        {/* Header — the tabs live here rather than on a row of their own. */}
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3">
          <button
            type="button"
            onClick={() => setFiltersOpen((v) => !v)}
            className="flex items-center gap-2 rounded-lg py-1.5 transition hover:opacity-80"
          >
          ...
          <div className="flex items-center gap-2">
            <div className="inline-flex rounded-xl border border-forest/15 bg-cream-2 p-1">
              <button
                onClick={() => setTab("all")}
      ...
      <div className="mb-3 flex items-center gap-2 text-sm text-clay">
        ...
        <span>{t("admin.studentsCount", { count: total })}</span>
        {isFetching && (
