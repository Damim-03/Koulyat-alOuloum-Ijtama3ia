      ...
      >
            ...
            <div>
              <h2 className="font-serif text-lg font-bold">
                {t("admin.editTopicTitle", {
                  defaultValue: "تعديل الموضوع المُسنَد",
                })}
              </h2>
              <p className="text-xs text-cream/70">
                {t("admin.editTopicSubtitle", {
                  defaultValue: "عدّل الأركان والطلبة والمشرف",
                })}
              </p>
            </div>
          </div>
        ...
        {busy ? (
          ...
          <div className="max-h-[70vh] space-y-5 overflow-y-auto p-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field
                label={t("admin.requirements", { defaultValue: "المتطلبات" })}
              >
                <textarea
                  value={requirements}
                  ...
                  placeholder={t("admin.onePerLine", {
                    defaultValue: "عنصر واحد في كل سطر",
                  })}
                />
              </Field>
              <Field label={t("admin.objectives", { defaultValue: "الأهداف" })}>
                <textarea
                  value={objectives}
                ...
                <div className="relative">
                  ...
                  <p className="mt-1.5 flex items-center gap-1 text-[10px] text-clay/80">
                    ...
                    {t("admin.unassignedOnlyHint", {
                      defaultValue: "يظهر هنا الطلبة غير المُسنَدين فقط",
                    })}
                  </p>
                      ...
                      {!searching && searchResults.length === 0 && (
                        <p className="px-3 py-3 text-center text-xs text-clay">
                          {t("admin.noUnassignedStudents", {
                            defaultValue:
                              "لا طالب غير مُسنَد مطابق — قد يكون له مشروع بالفعل",
                          })}
                        </p>
                      )}
        ...
        <div className="flex items-center justify-start gap-2 rounded-b-2xl border-t border-forest/10 px-6 py-4">
          ...
          >
            ...
            {t("admin.saveChanges", { defaultValue: "حفظ التعديلات" })}
          </button>
          <button
            type="button"
            onClick={onClose}
