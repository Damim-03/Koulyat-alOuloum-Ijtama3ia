    ...
    >
      {isLoading ? (
        ...
        <div className="space-y-5">
          {/* Members */}
          <section>
            <h4 className="mb-2 text-xs font-bold text-forest">
              {t("admin.members")}
            </h4>
            <div className="flex flex-wrap gap-2">
              {members.length === 0 && (
                <p className="text-xs text-clay">{t("admin.noMembers")}</p>
              )}
              {members.map((m) => (
                <div
                  key={m.id}
          ...
          <section>
            ...
            <div className="space-y-2">
              ...
              {project.milestones?.map((ms) => (
                <div
                  key={ms.id}
                  className="flex items-center justify-between rounded-xl bg-cream-2 px-3 py-2"
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-clay" dir="ltr">
                      {ms.deadline
                        ? new Date(ms.deadline).toLocaleDateString(i18n.language)
                        : ""}
                    </span>
                    <span
                      className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${MS_STYLES[ms.status] ?? "bg-gray-100 text-gray-600"}`}
                    >
